...
exports.Auth = function (req, res, next) {
    ...
    try {
        ...
        if(payload.exp <= moment().unix()){ return res.status(500).send({ mensaje: "El token ha expirado."});
        }
    } catch (error) { return res.status(500).send({ mensaje: "El token no es valido."})
    }
